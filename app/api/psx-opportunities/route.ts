...
async function fetchMonthly(symbol: string): Promise<MonthlyResult | null> {
  ...
  const averageVolume =
    window.reduce((total, point) => total + point.volume, 0) / window.length;
  const latestVolume = window[window.length - 1].volume;
  const volumeRatio = averageVolume > 0 ? latestVolume / averageVolume : null;
  const positiveDays = window.filter(
    (point, index) => index === 0 || point.close >= window[index - 1].close,
  ).length;

  const returnScore = Math.min(
    40,
    Math.max(0, (monthlyChangePercent + 5) * 2.5),
  );
  const consistencyScore = (positiveDays / window.length) * 35;
  const volumeScore =
    volumeRatio == null
      ? 0
      : Math.min(25, Math.max(0, (volumeRatio - 0.5) * 12.5));
  const score = returnScore + consistencyScore + volumeScore;
  const reasons: string[] = [];

  if (monthlyChangePercent >= 5)
    reasons.push(`${monthlyChangePercent.toFixed(1)}% monthly uptrend`);
  else if (monthlyChangePercent > 0)
    reasons.push(`${monthlyChangePercent.toFixed(1)}% monthly gain`);
  if (positiveDays / window.length >= 0.6)
    reasons.push(`${positiveDays}/${window.length} positive sessions`);
  if (volumeRatio != null && volumeRatio >= 1.25)
  ...
  try {
    ...
    if (!stocks.length) {
      return NextResponse.json(
        { success: false, error: "Stocks are required" },
        { status: 400 },
      );
    }
    ...
    for (let index = 0; index < stocks.length; index += batchSize) {
      ...
      const settled = await Promise.allSettled(
        batch.map((stock) => fetchMonthly(stock.symbol.toUpperCase())),
      );
