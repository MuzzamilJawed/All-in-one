...
export function normalizeTxn(body: any): NormalizedTxn | { error: string } {
  ...
  const brokerage =
    body?.brokerage === "" || body?.brokerage == null
      ? 0
      : Number(body.brokerage);
  ...
  if (!CURRENCIES.includes(body?.currency))
    return { error: "Currency must be PKR or USD" };
  if (!Number.isFinite(quantity) || quantity <= 0)
    return { error: "Quantity must be a positive number" };
  if (!Number.isFinite(price) || price <= 0)
    return { error: "Price must be a positive number" };
  if (!Number.isFinite(brokerage) || brokerage < 0)
    return { error: "Brokerage must be zero or a positive number" };
  if (brokerageMode === "PERCENT" && brokerage > 100)
    return { error: "Brokerage percentage cannot exceed 100" };
